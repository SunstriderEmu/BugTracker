...
class instance_ruins_of_ahn_qiraj : public InstanceMapScript
{
public:
    instance_ruins_of_ahn_qiraj() : InstanceMapScript("instance_ruins_of_ahnqiraj", 509) { }

    InstanceScript* GetInstanceScript(InstanceMap* map) const override
    {
        return new instance_ruins_of_ahn_qiraj_script(map);
    }

    struct instance_ruins_of_ahn_qiraj_script : public InstanceScript
    {
        instance_ruins_of_ahn_qiraj_script(Map* pMap) : InstanceScript(pMap) { Initialize(); }
        ...
        {
            for (unsigned char i : m_auiEncounter)
                if (i == IN_PROGRESS) return true;

            return false;
        }

        void OnCreatureCreate(Creature* pCreature, uint32) override
        {
...
{
    new instance_ruins_of_ahn_qiraj();
}
