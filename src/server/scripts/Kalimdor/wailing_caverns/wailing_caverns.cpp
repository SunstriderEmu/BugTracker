...
bool GossipHello_npc_disciple_of_naralex(Player* pPlayer, Creature* pCreature)
{
    ...
    if (pInstance)
    {
            ...
            (pInstance->GetData(TYPE_LADY_ANACONDRA) == DONE) && (pInstance->GetData(TYPE_LORD_SERPENTIS) == DONE))
        {
            ...
            pPlayer->SEND_GOSSIP_MENU_TEXTID(GOSSIP_ID_START_2, pCreature->GetGUID());
        ...
        {
            pPlayer->SEND_GOSSIP_MENU_TEXTID(GOSSIP_ID_START_1, pCreature->GetGUID());
        }
    }
    return true;
}
...
void AddSC_wailing_caverns()
{
    ...
    newscript->OnGossipHello =  &GossipHello_npc_disciple_of_naralex;
    newscript->OnGossipSelect = &GossipSelect_npc_disciple_of_naralex;
