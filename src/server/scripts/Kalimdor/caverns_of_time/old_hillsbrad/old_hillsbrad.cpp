...
bool GossipSelect_npc_brazen(Player *player, Creature *_Creature, uint32 sender, uint32 action )
{
    if (action == GOSSIP_ACTION_INFO_DEF+1)
    {
        if( !player->HasItemCount(ITEM_ENTRY_BOMBS,1) )
            player->SEND_GOSSIP_MENU_TEXTID(9780, _Creature->GetGUID());
        else
        {
...
bool GossipHello_npc_erozion(Player *player, Creature *_Creature)
{
    ...
    player->SEND_GOSSIP_MENU_TEXTID(9778, _Creature->GetGUID());
...
bool GossipSelect_npc_erozion(Player *player, Creature *_Creature, uint32 sender, uint32 action)
{
    if( action == GOSSIP_ACTION_INFO_DEF+1 )
    {
        ...
        player->SEND_GOSSIP_MENU_TEXTID(9515, _Creature->GetGUID());
    }
...
{
    auto  thrall_walkAI = new npc_thrall_old_hillsbradAI(_Creature);
...
bool GossipHello_npc_thrall_old_hillsbrad(Player *player, Creature *_Creature)
{
    ...
    if( pInstance )
    {
        if (pInstance->GetData(TYPE_BARREL_DIVERSION) == DONE && !pInstance->GetData(TYPE_THRALL_EVENT))
        {
            ...
            player->SEND_GOSSIP_MENU_TEXTID(GOSSIP_ID_START, _Creature->GetGUID());
        }
        ...
        if( pInstance->GetData(TYPE_THRALL_PART1) == DONE && !pInstance->GetData(TYPE_THRALL_PART2) )
        {
            ...
            player->SEND_GOSSIP_MENU_TEXTID(GOSSIP_ID_SKARLOC1, _Creature->GetGUID());
        }
        ...
        if( pInstance->GetData(TYPE_THRALL_PART2) == DONE && !pInstance->GetData(TYPE_THRALL_PART3) )
        {
            ...
            player->SEND_GOSSIP_MENU_TEXTID(GOSSIP_ID_TARREN, _Creature->GetGUID());
        }
    }
    return true;
}
...
bool GossipSelect_npc_thrall_old_hillsbrad(Player *player, Creature *_Creature, uint32 sender, uint32 action)
{
    ...
    switch( action )
    {
        ...
        case GOSSIP_ACTION_INFO_DEF+2:
            ...
            player->SEND_GOSSIP_MENU_TEXTID(GOSSIP_ID_SKARLOC2, _Creature->GetGUID());
            break;

        case GOSSIP_ACTION_INFO_DEF+20:
            player->SEND_GOSSIP_MENU_TEXTID(GOSSIP_ID_SKARLOC3, _Creature->GetGUID());
...
{
    auto  taretha_walkAI = new npc_tarethaAI(_Creature);
...
bool GossipHello_npc_taretha(Player *player, Creature *_Creature)
{
    ...
    if( pInstance && pInstance->GetData(TYPE_THRALL_PART3) == DONE && pInstance->GetData(TYPE_THRALL_PART4) == NOT_STARTED)
    {
        ...
        player->SEND_GOSSIP_MENU_TEXTID(GOSSIP_ID_EPOCH1, _Creature->GetGUID());
    }
    return true;
}
...
bool GossipSelect_npc_taretha(Player *player, Creature *_Creature, uint32 sender, uint32 action)
{
    ...
    if( action == GOSSIP_ACTION_INFO_DEF+1 )
    {
        ...
        player->SEND_GOSSIP_MENU_TEXTID(GOSSIP_ID_EPOCH2, _Creature->GetGUID());
    }
    if( action == GOSSIP_ACTION_INFO_DEF+2 )
    {
        player->CLOSE_GOSSIP_MENU();
        ...
        if( pInstance->GetData(TYPE_THRALL_EVENT) == IN_PROGRESS )
        {
            ...
            if(pInstance->GetData64(DATA_EPOCH) == 0)
             ...
             {
                 Creature* Thrall = (ObjectAccessor::GetCreature((*_Creature), ThrallGUID));
                 if(Thrall)
...
void AddSC_old_hillsbrad()
{
    ...
    newscript->OnGossipHello =   &GossipHello_npc_brazen;
    newscript->OnGossipSelect =  &GossipSelect_npc_brazen;
    sScriptMgr->RegisterOLDScript(newscript);

    newscript = new OLDScript;
    newscript->Name="npc_erozion";
    newscript->OnGossipHello =   &GossipHello_npc_erozion;
    newscript->OnGossipSelect =  &GossipSelect_npc_erozion;
    sScriptMgr->RegisterOLDScript(newscript);

    newscript = new OLDScript;
    newscript->Name="npc_thrall_old_hillsbrad";
    newscript->OnGossipHello =  &GossipHello_npc_thrall_old_hillsbrad;
    newscript->OnGossipSelect = &GossipSelect_npc_thrall_old_hillsbrad;
    newscript->GetAI = &GetAI_npc_thrall_old_hillsbrad;
    sScriptMgr->RegisterOLDScript(newscript);

    newscript = new OLDScript;
    newscript->Name="npc_taretha";
    newscript->OnGossipHello =   &GossipHello_npc_taretha;
    newscript->OnGossipSelect =  &GossipSelect_npc_taretha;
