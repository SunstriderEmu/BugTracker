...
protected:
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action) override
    {
        ...
        {
        case MENU_EQUIPMENT:
            SelectInEquipmentMenu(player, creature, action);
            break;
        case MENU_EQUIPMENT_SET:
            SelectInEquipmentSetMenu(player, creature, action);
            break;
        case MENU_SPELLS:
            SelectInSpellsMenu(player, creature, action);
            break;
        case MENU_LEVEL:
            SelectInLevelMenu(player, creature, action);
            break;
        case MENU_SKILLS:
            SelectInSkillsMenu(player, creature, action);
            break;
        case MENU_PROFESSIONS:
            SelectInProfessionsMenu(player, creature, action);
            break;
        case MENU_TELEPORT:
            SelectInTeleportMenu(player, creature, action);
            break;
        case MENU_MAIN:
        default:
            SelectInMainMenu(player, creature, action);
            break;
        }
        return true;
    }
