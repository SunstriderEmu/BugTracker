...
enum COG_GameEvent
{
    GAME_EVENT_CHILDEREN_OF_GOLDSHIRE = 60
};
...
struct npc_cameron : public ScriptedAI
{
    ...
    void WaypointReached(uint32 waypointId, uint32 pathId) override
    {
        ...
        case STORMWIND_PATH:
        {
            if (waypointId == STORMWIND_WAYPOINT)
            {
                ...
                _events.RescheduleEvent(EVENT_WP_START_GOLDSHIRE, 11min);
            }
        ...
        case GOLDSHIRE_PATH:
        {
            ...
            {
                me->GetMotionMaster()->MoveRandom(10.f);
                _events.RescheduleEvent(EVENT_WP_START_WOODS, 15min);
            }
            break;
        }
        case WOODS_PATH:
        {
            ...
            {
                me->GetMotionMaster()->MoveRandom(10.f);
                _events.RescheduleEvent(EVENT_WP_START_HOUSE, 6min);
                _events.RescheduleEvent(EVENT_WP_START_LISA, 2s);
            }
        ...
        case HOUSE_PATH:
        {
            if (waypointId == HOUSE_WAYPOINT)
            {
                ...
                _events.RescheduleEvent(EVENT_PLAY_SOUNDS, 30s);
            }
            break;
        }
        }
    ...
    void OnGameEvent(bool start, uint16 eventId) override
    {
        if (start && eventId == GAME_EVENT_CHILDEREN_OF_GOLDSHIRE)
        {
            // Start event at 7 am
            // Begin pathing
            _events.RescheduleEvent(EVENT_BEGIN_EVENT, 2s);
            _started = true;
        }
