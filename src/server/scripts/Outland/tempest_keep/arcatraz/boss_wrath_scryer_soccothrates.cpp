    ...
    public:
        ...
        struct boss_wrath_scryer_soccothratesAI : public BossAI
        {
            ...
            void Reset() override
            {
                ...
                dalliahTaunt = false;
                dalliahDeath = false;
                DoCast(me, IsHeroic() ? SPELL_FEL_IMMOLATION_H : SPELL_FEL_IMMOLATION);
            }
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_FELFIRE_SHOCK:
                            DoCastVictim(IsHeroic() ? SPELL_FELFIRE_SHOCK_H : SPELL_FELFIRE_SHOCK, true);
                            events.ScheduleEvent(EVENT_FELFIRE_SHOCK, urand(12000, 14000));
                            break;
                ...
                if (me->HealthBelowPct(25) && !dalliahTaunt)
                {
                    if (Creature* dalliah = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_DALLIAH)))
