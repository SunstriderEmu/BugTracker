...
- Last phase (flying part) not working yet
*/

//Advisors AI are done in SmartAI
...
class boss_kaelthas : public CreatureScript
{
    public:
        ...
        {
            boss_kaelthasAI(Creature* creature) : BossAI(creature, DATA_KAELTHAS_EVENT) { }
            ...
            void PrepareAdvisors()
            {
                ...
                for (uint8 i = DATA_THALADREDTHEDARKENER; i <= DATA_MASTERENGINEERTELONICUS; ++i)
                    if (Creature* advisor = ObjectAccessor::GetCreature(*me, ObjectGuid(instance->GetData64(i))))
                    {
                        ...
                        advisor->SetSheath(SHEATH_STATE_MELEE);
            ...
            void Reset() override
            {
                //do not despawn summons BossAI::Reset();
                events2.Reset();
            ...
            {
                return me->GetHomePosition().GetExactDist2d(me) > 165.0f || !me->AI()->SelectTarget(SELECT_TARGET_RANDOM, 0.0f, 165.0f, true);
            }
